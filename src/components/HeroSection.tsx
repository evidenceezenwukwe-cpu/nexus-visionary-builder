import { Button } from "@/components/ui/button";
import { ArrowRight, Download, Users } from "lucide-react";
import professionalHeadshot from "@/assets/professional-headshot.webp";

const HeroSection = () => {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-background to-background pt-20 lg:pt-0">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,hsl(var(--primary)/0.1)_0%,transparent_50%)]" />

      <div className="section-container relative z-10 w-full overflow-x-hidden">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-20 items-center w-full">

          {/* Image — TOP on mobile, right column on desktop */}
          <div className="relative flex justify-center lg:justify-end order-first lg:order-last w-full">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-tr from-primary/20 to-accent/20 rounded-full blur-3xl animate-pulse" />
              <img
                src={professionalHeadshot}
                alt="Evidence Ezenwukwe"
                className="relative z-10 w-48 h-48 sm:w-64 sm:h-64 lg:w-96 lg:h-96 object-cover rounded-full shadow-2xl"
              />
            </div>
          </div>

          {/* Content */}
          <div className="flex flex-col items-center lg:items-start text-center lg:text-left space-y-6 lg:space-y-8 w-full">
            <div className="space-y-4 lg:space-y-6 w-full">
              <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-display font-light leading-[0.9]">
                <span className="block text-foreground font-extralight">Evidence</span>
                <span className="block bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent font-medium">
                  Ezenwukwe
                </span>
              </h1>

              <div className="space-y-2 w-full">
                <p className="text-lg sm:text-xl lg:text-2xl text-primary font-medium tracking-wide uppercase">
                  Lawyer • Entrepreneur • Builder
                </p>
                <p className="text-base sm:text-lg lg:text-xl text-muted-foreground font-light leading-relaxed max-w-2xl mx-auto lg:mx-0">
                  Nigerian Law School student (Enugu Campus), founder of Evicom Concept, and builder of Agbani Runs —
                  empowering Africa through pragmatic education, technology, and purpose-driven entrepreneurship.
                </p>
              </div>
            </div>

            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-center lg:items-start justify-center lg:justify-start w-full">
              <Button
                size="lg"
                className="btn-hero group w-full sm:w-auto"
                onClick={() => document.getElementById('work')?.scrollIntoView({ behavior: 'smooth' })}
              >
                Explore My Work
                <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Button>
              <Button variant="outline" size="lg" className="btn-hero-outline w-full sm:w-auto">
                <Download className="mr-2 w-5 h-5" />
                Download E-Book
              </Button>
              <Button
                variant="outline"
                size="lg"
                className="btn-hero-outline w-full sm:w-auto"
                onClick={() => document.getElementById('mission')?.scrollIntoView({ behavior: 'smooth' })}
              >
                <Users className="mr-2 w-5 h-5" />
                Join My Mission
              </Button>
            </div>

            <div className="flex flex-wrap gap-6 sm:gap-8 justify-center lg:justify-start pt-4 w-full">
              <div className="text-center">
                <div className="text-2xl sm:text-3xl font-bold text-primary">500+</div>
                <div className="text-xs sm:text-sm text-muted-foreground">Students Taught</div>
              </div>
              <div className="text-center">
                <div className="text-2xl sm:text-3xl font-bold text-primary">2</div>
                <div className="text-xs sm:text-sm text-muted-foreground">E-Books Published</div>
              </div>
              <div className="text-center">
                <div className="text-2xl sm:text-3xl font-bold text-primary">2+</div>
                <div className="text-xs sm:text-sm text-muted-foreground">Apps Launched</div>
              </div>
            </div>
          </div>

        </div>
      </div>
    </section>
  );
};

export default HeroSection;
